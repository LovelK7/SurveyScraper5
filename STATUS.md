...
| 2.1e — sastavnica (title block) | **DESIGN ONLY (2026-09-19)** — the Illustrator route's title block, prefilled from SB + the leaf's filled OSZ + the geo finders into `SB_<broj>_sastavnica.pdf` beside the cave's OSZ. Template measured and the render mechanism validated by spike (content-stream strip of the 15 placeholder values → centred shrink-to-fit text; Myriad Pro reproduces the authored widths to 0.01 pt). Nothing built; 8 open questions in [sastavnica-design.md](features/cave-dossier/docs/sastavnica-design.md). Outside the M-ladder — needs only M1 |
