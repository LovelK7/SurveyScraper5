...
Updated: 2026-09-20 (maintained by `/wrap-up` at the end of each session)
...
- 2026-09-20 (evening) — **KORAK 3 shipped**: five delegated tasks (T4 layout chooser, T1 XML finisher, T2 headless driver, T3 `cavedossier nacrt` compositor + template v1.0, T5 launcher) reviewed and committed; prod v1.5 and csx kit v1.3 published; second cave SB 1256 moved two rules (bar-length fixed point, wider Dislivello shift). Project 0004 at `validation`, one backlog check left → [journal/SESSIONS.md](journal/SESSIONS.md)
- 2026-09-20 — **Nacrt finishing explored, headless cSurvey proven**: the user's own SB 1103 session (raw `_lt` vs autosave) gave the exact XML of every manual step (entrance flag, Dislivello quota, horizontal-scale quota, compass item `m=1`, A4 1:100 print options); three source digs pinned the semantics (`scalemode` is a combo index, render quality lives in `sharedsettings`, centering is hard-coded, one design per sheet). Then the installed `cSurveyPC.exe` was loaded from Windows PowerShell via reflection — `Load` → Friend `Calculate` → `SaveTo` and an unseen `frmPreview` printing both designs to "Microsoft Print to PDF" as files. Project 0004 brief carries the automation matrix + five delegable task prompts → [journal/SESSIONS.md](journal/SESSIONS.md)
