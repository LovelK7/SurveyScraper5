...
**Verified 2026-09-20 — the reflection route works from Windows PowerShell 5.1 with no build at all.** `Assembly.LoadFrom("C:\\csurvey64\\cSurveyPC.exe")`, then three pokes replace the WinForms startup: set the private `modMain.sApplicationPath` to the install dir (so `GetApplicationPath` stops pointing at powershell.exe), call `modMain.LoadLocalizedStrings`, and on `My.MyProject.Application` call `ReloadSettings()` and set `oRuntimeSettings` to a fresh `cEnvironmentSettings`. After that `New cSurvey` → `Load` → `Invalidate` → Friend `Calculate.Calculate(True)` (via `GetMethod` with `NonPublic`) → `SaveTo` all run, and `frmPreview` can be constructed unseen and printed to a PDF file (see [exports-and-printing.md](exports-and-printing.md)). Script: [projects/0004-nacrt-finishing/findings/csurvey_headless_probe.ps1](../projects/0004-nacrt-finishing/findings/csurvey_headless_probe.ps1). Requires `-STA`; PowerShell variable names are case-insensitive (`$survey` vs `$Survey` collide).
