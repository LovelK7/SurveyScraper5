# SurveyScraper4 — first-session setup prompt

> Paste the block below into a fresh Claude Code session opened **in this
> `SurveyScraper4` workspace**. It is self-contained — the new session needs no
...
You are setting up a brand-new, almost-empty repo: **SurveyScraper4** (the
"superapp") at `C:\Users\Lovel.IZRK-LK-NB\Programming\SurveyScraper4`. Your job
is to scaffold it and migrate one existing body of work into it as a single
feature. Work only inside SurveyScraper4 — the other two repos are read-only
reference material.

## Mental model

SurveyScraper4 is an **umbrella "superapp"** that will unify several
...
The **first feature** of SurveyScraper4 is the TopoDroid → finished-map pipeline
work that currently lives in `cSurvey\dev\`. In SurveyScraper4 it becomes **one
feature named `csx-to-survey-pipeline`** — it is NOT the foundation of the
superapp, just its first module.

## Tasks

1. **Multi-root workspace.** Create `SurveyScraper4.code-workspace` at the repo
   root with three folders: this repo (SurveyScraper4), `../cSurvey` (labeled
...
4. **Write the superapp `CLAUDE.md`** at the repo root, covering: what
   SurveyScraper4 is (umbrella superapp); the feature list (just
...
6. **git init + one commit.** `git init`, stage everything, single initial
   commit: `Initial SurveyScraper4 scaffold: csx-to-survey-pipeline migrated from cSurvey/dev`.
...
- `cSurvey` and `crospeleo-automation` are **READ-ONLY**. Never edit them, never
  run git write commands in them. All work lands in **SurveyScraper4**.
...
Report: the final tree of SurveyScraper4, how many citations you rewrote and the
