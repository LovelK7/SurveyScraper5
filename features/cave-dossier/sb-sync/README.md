...
`1-za-sb.tsv` carries **all 24 columns of `Svi objekti`, in the workbook's own
order**, with empties where the satellite has nothing — a subset cannot be pasted
into a table. Every file is written UTF-8 **with a BOM**, because Windows Excel
reads a UTF-8 file as the local codepage without one and turns every č/š/ž into
mojibake.
